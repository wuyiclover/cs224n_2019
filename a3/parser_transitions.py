...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###
        self.stack = ["ROOT"]
        self.buffer = self.sentence.copy()   ##直接self.buffer =sentence.copy会显示sentence不能被修改；"A=B"，改变了A，B也会变;
        self.dependencies = []               ##下面说的深浅拷贝问题， A=B[:]浅拷贝 or A=B.copy()  两个互不影响
...
def minibatch_parse(sentences, model, batch_size):
    ...
    @param model (ParserModel): The model that makes parsing decisions. It is assumed to have a function
                                ...
                                #每次参数是list of PartialParses,每个parse的next transition的一个集合tran
                                #（len(parse)==len(tran)），每个tran对应每个parse的next transition。
    ...
    ###                 unfinished_parses = partial_parses[:].  两个改变不互相影响
    ...
    while len(unfinished_parses):
        ...
        #print(len(transitions) == len(unfinished_parses[:batch_size]))
        #print(transitions)
        #print("*" * 20)
        for tran, parse in zip(transitions, unfinished_parses[:batch_size]):
            #print(tran)
            #print("*" * 20)
